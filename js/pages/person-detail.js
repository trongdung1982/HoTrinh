...
// ============================================================
//
// Ba cửa: openPersonMenu() (LÀM GÌ) · openPersonDetail() (LÀ AI) ·
// openUnionDetail() (GIA ĐÌNH RA SAO) — vì sao tách: sổ tay.
// ⚠ Hai cửa, MỘT bộ `xuLy` đi xuyên qua cả hai màn hình.
// ⚠ KHÔNG import `tree-view.js` (vòng tròn module) — báo ra bằng `onChonNguoi`.
// ⚠ Trường trống ẩn cả hàng; công tắc "Còn N mục chưa điền" vẽ lại cả bảng.
// ⚠ Người vành đai chỉ kể tên, không bấm được — `timNguoiThe()`.
...
/**
 * Người trong cây, hoặc người VÀNH ĐAI (b127b, `THIET-KE-NHIEU-CAY.md` mục 6):
 * ngoài cây nhưng có dây nối thẳng vào người trong cây. Người vành đai chỉ
 * được KỂ TÊN trên thẻ — không vẽ, không thẻ riêng, không bấm tới được.
 */
...
/** Con của một cặp mà thẻ kể được — trong cây hoặc ở vành đai. */
...
function veNhom(tieuDe, danhSach, xuLy, ...cacNutPhu) {
  ...
  for (const muc of danhSach) {
    ...
    const song = doiSongNguoi(p);
    // Người vành đai: một dòng chữ, KHÔNG phải nút — họ không có thẻ để mở.
