...
// muốn nới trần — sửa bảng dưới đây và ghi lý do vào nhật ký bước.
...
for (const p of fileJs) {
  ...
  if (dong.length > TRAN.maDai) xem.push(`${dong.length} dòng · ${rel} (cân nhắc tách — V7)`);
}
...
// 3. INDEX: số dòng dài quá trần (file chỉ được thêm dòng → con số này chỉ được đứng yên)
{
  const dong = cacDong(docChu(path.join(REPO, 'nhat-ky', 'INDEX.md')));
...
// 4. Tài liệu ở gốc repo sai quy ước tên
{
